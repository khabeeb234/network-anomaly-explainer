"""
utils.py — Network Anomaly Explainer (Final, Rate-Limit-Safe)
-------------------------------------------------------------
Features:
 - IsolationForest anomaly detection
 - Gemini 2.5 Flash → auto fallback to Pro model
...
import datetime
import logging
import time   # 👈 for API delay handling
...
# ===============================================================
# 4️⃣ GEMINI AI EXPLANATION (Flash → Pro Fallback)
# ===============================================================
...
Give a concise, 3–4 sentence summary:
1️⃣ Likely cause (general)
2️⃣ User impact
...
"""
    ...
    if use_llm and gemini_key:
        try:
            ...
            if not output:
                print("⚠️ Retrying with Gemini 2.5 Pro due to filtered/empty output…")
...
def check_gemini_connection():
    try:
        ...
        gemini_key = os.environ.get("GEMINI_API_KEY")
        if not gemini_key:
            return False, "⚠️ GEMINI_API_KEY not set — using rule-based fallback."
        genai.configure(api_key=gemini_key)
        model = genai.GenerativeModel("gemini-2.5-flash")
        _ = model.generate_content("Hello from Gemini AI!")
        return True, "✅ Connected to Gemini 2.5 Flash"
    except Exception as e:
        return False, f"⚠️ Gemini connection failed: {e}"
